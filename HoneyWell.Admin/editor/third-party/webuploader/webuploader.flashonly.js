...
(function( root, factory ) {
    ...
    define('runtime/runtime',[
        ...
        Runtime.create = function( opts, order ) {
            var type, runtime;
    
            order = order || Runtime.order;
            $.each( order.split( /\s*,\s*/g ), function() {
                if ( factories[ this ] ) {
                    type = this;
                    return false;
                }
    ...
    define('widgets/runtime',[
        ...
        return Uploader.register({
            ...
            predictRuntmeType: function() {
                var order = this.options.runtimeOrder || Runtime.order,
                    type = this.type,
                    i, len;
    
                if ( !type ) {
                    order = order.split( /\s*,\s*/g );
    
                    for ( i = 0, len = order.length; i < len; i++ ) {
                        if ( Runtime.hasRuntime( order[ i ] ) ) {
                            this.type = type = order[ i ];
                            break;
                        }
                    }
                }
