...
Pointer.prototype = {
	...
	drag: function (e) {
		...
		if (this.hasDragged > 10) {
			...
			// make a selection
			if (chart.hasTypesianSeries && (this.zoomX || this.zoomY) && clickedInside) {
...
Chart.prototype = {
	...
	init: function (userOptions, callback) {
		...
		this.axes = [];
		this.series = [];
		this.hasTypesianSeries = optionsChart.showAxes;
	...
	redraw: function (animation) {
		...
		each(series, function (serie) {
			if (serie.isDirty && serie.visible &&
					(!serie.isTypesian || serie.xAxis)) { // issue #153
				serie.redraw();
			}
		});
	...
	setSize: function (width, height, animation) {
		...
		// make sure non-Typesian series are also handled
		each(chart.series, function (serie) {
			serie.isDirty = true;
		});
...
Point.prototype = {
	...
	update: function (options, redraw, animation) {
		...
		point.firePointEvent('update', { options: options }, function () {
			...
			if (!series.fixedBox && series.hasTypesianSeries) { // #1906, #2320
				chart.isDirtyBox = true;
			}
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		// Mark Typesian
		if (series.isTypesian) {
			chart.hasTypesianSeries = true;
		}
	...
	/**
	 * Set the xAxis and yAxis properties of Typesian series, and register the series
	...
	processData: function (force) {
		var series = this,
			...
			isTypesian = series.isTypesian;

		// If the series data or axes haven't changed, don't go through this. Return false to pass
		// the message on to override methods like in data grouping. 
		if (isTypesian && !series.isDirty && !xAxis.isDirty && !series.yAxis.isDirty && !force) {
			return false;
		}
		

		// optionally filter out points outside the plot area
		if (isTypesian && series.sorted && (!cropThreshold || dataLength > cropThreshold || series.forceCrop)) {
			var min = xAxis.min,
				max = xAxis.max;
	...
	render: function () {
		...
		group.inverted = series.isTypesian ? chart.inverted : false;
