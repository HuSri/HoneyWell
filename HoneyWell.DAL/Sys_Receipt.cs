...
namespace HoneyWell.DAL
{
	...
	{ 
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from Sys_Receipt");
			strSql.Append(" where ");
			                                       strSql.Append(" ID = @ID  ");
		...
		public int Add(Model.Sys_Receipt model)
		{
			...
			strSql.Append("insert into  Sys_Receipt(");
		...
		public bool Update(Model.Sys_Receipt model)
		{
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<SqlParameter> paras = new List<SqlParameter>();
			strSql.Append("update Sys_Receipt set ");
            foreach (PropertyInfo pi in pros)
            {
			    //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("ID"))
		...
		public Model.Sys_Receipt GetModel(int ID)
		{
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.Sys_Receipt model = new Model.Sys_Receipt();
			//利用反射获得属性的所有公共属性
            PropertyInfo[] pros = model.GetType().GetProperties();
            ...
            strSql.Append(" from Sys_Receipt");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.Sys_Receipt DataRowToModel(DataRow row)
        {
            Model.Sys_Receipt model = new Model.Sys_Receipt();
            if (row != null)
            {
