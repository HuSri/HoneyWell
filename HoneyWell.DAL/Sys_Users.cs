...
namespace HoneyWell.DAL
{
	...
	{ 
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Model.Sys_Users model)
		{
           	StringBuilder strSql = new StringBuilder();
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Model.Sys_Users model)
		{
			StringBuilder strSql = new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.Sys_Users GetModel(int ID)
		{
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.Sys_Users model = new Model.Sys_Users();
			//利用反射获得属性的所有公共属性
            PropertyInfo[] pros = model.GetType().GetProperties();
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.Sys_Users DataRowToModel(DataRow row)
        {
            Model.Sys_Users model = new Model.Sys_Users();
            if (row != null)
            {
