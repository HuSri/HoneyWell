...
namespace HoneyWell.DAL
{
	...
	{ 
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,MenuNameC,MenuNameE from Sys_Menu ");
            strSql.Append(" where 1=1 " + SqlWhere + "");

            HoneyWell.Model.Sys_Menu model = new HoneyWell.Model.Sys_Menu();
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,MenuCode,MenuNameC,MenuNameE,ParentID,MenuLevel,MenuOrder,MenuUrl,MenuDesc  from Sys_Menu ");
            strSql.Append(" where 1=1 " + SqlWhere + "");
            HoneyWell.Model.Sys_Menu model = new HoneyWell.Model.Sys_Menu();
            DataSet ds = SqlHelper.Query(strSql.ToString(), SysConn.PublicConn);
            if (ds != null)
