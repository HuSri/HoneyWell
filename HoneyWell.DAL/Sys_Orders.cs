...
namespace HoneyWell.DAL
{
	...
	{ 
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from Sys_Orders");
			strSql.Append(" where ");
			                                       strSql.Append(" ID = @ID  ");
		...
		public int Add(Model.Sys_Orders model)
		{
			...
			strSql.Append("insert into  Sys_Orders(");
		...
		public bool Update(Model.Sys_Orders model)
		{
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<SqlParameter> paras = new List<SqlParameter>();
			strSql.Append("update Sys_Orders set ");
            foreach (PropertyInfo pi in pros)
            {
			    //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("ID"))
		...
		public Model.Sys_Orders GetModel(int ID)
		{
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.Sys_Orders model = new Model.Sys_Orders();
			//利用反射获得属性的所有公共属性
            PropertyInfo[] pros = model.GetType().GetProperties();
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.Sys_Orders DataRowToModel(DataRow row)
        {
            Model.Sys_Orders model = new Model.Sys_Orders();
            if (row != null)
            {
