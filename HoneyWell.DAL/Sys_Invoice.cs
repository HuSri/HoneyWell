...
namespace HoneyWell.DAL
{
	...
	{ 
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from Sys_Invoice");
			strSql.Append(" where ");
			                                       strSql.Append(" ID = @ID  ");
		...
		public int Add(Model.Sys_Invoice model)
		{
			...
			strSql.Append("insert into  Sys_Invoice(");
		...
		public bool Update(Model.Sys_Invoice model)
		{
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<SqlParameter> paras = new List<SqlParameter>();
			strSql.Append("update Sys_Invoice set ");
            foreach (PropertyInfo pi in pros)
            {
			    //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("ID"))
		...
		public Model.Sys_Invoice GetModel(int ID)
		{
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.Sys_Invoice model = new Model.Sys_Invoice();
			//利用反射获得属性的所有公共属性
            PropertyInfo[] pros = model.GetType().GetProperties();
            ...
            strSql.Append(" from Sys_Invoice");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.Sys_Invoice DataRowToModel(DataRow row)
        {
            Model.Sys_Invoice model = new Model.Sys_Invoice();
            if (row != null)
            {
