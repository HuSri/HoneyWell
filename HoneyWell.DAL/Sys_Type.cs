...
namespace HoneyWell.DAL
{
	...
	{ 
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from Sys_Type");
			strSql.Append(" where ");
			                                       strSql.Append(" ID = @ID  ");
		...
		public int Add(Model.Sys_Type model)
		{
			...
			strSql.Append("insert into  Sys_Type(");
		...
		public bool Update(Model.Sys_Type model)
		{
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<SqlParameter> paras = new List<SqlParameter>();
			strSql.Append("update Sys_Type set ");
            foreach (PropertyInfo pi in pros)
            {
			    //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("ID"))
		...
		public Model.Sys_Type GetModel(int ID)
		{
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.Sys_Type model = new Model.Sys_Type();
			//利用反射获得属性的所有公共属性
            PropertyInfo[] pros = model.GetType().GetProperties();
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.Sys_Type DataRowToModel(DataRow row)
        {
            Model.Sys_Type model = new Model.Sys_Type();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,TName from Sys_Type ");
            strSql.Append(" where 1=1 " + SqlWhere + "");

            HoneyWell.Model.Sys_Type model = new HoneyWell.Model.Sys_Type();
            DataSet ds = SqlHelper.Query(strSql.ToString(), SysConn.PublicConn);
            if (ds != null)
            {
                return ds;
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,TCode,TName,TSmallPic,TLevel,ParentId,TOrder from Sys_Type ");
            strSql.Append(" where 1=1 " + SqlWhere + "");
            HoneyWell.Model.Sys_Type model = new HoneyWell.Model.Sys_Type();
            DataSet ds = SqlHelper.Query(strSql.ToString(), SysConn.PublicConn);
            if (ds != null)
