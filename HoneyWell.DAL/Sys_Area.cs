...
namespace HoneyWell.DAL
{
	...
	{ 
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from Sys_Area");
			strSql.Append(" where ");
			                                       strSql.Append(" ID = @ID  ");
		...
		public Model.Sys_Area GetModel(int ID)
		{
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.Sys_Area model = new Model.Sys_Area();
			//利用反射获得属性的所有公共属性
            PropertyInfo[] pros = model.GetType().GetProperties();
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.Sys_Area DataRowToModel(DataRow row)
        {
            Model.Sys_Area model = new Model.Sys_Area();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,AreaCode,AreaName,ParentAreaCode,AreaLevel,AreaOrder,KeyWord  from Sys_Area");
            strSql.Append(" where 1=1 " + SqlWhere + "");
            HoneyWell.Model.Sys_Type model = new HoneyWell.Model.Sys_Type();
            DataSet ds = SqlHelper.Query(strSql.ToString(), SysConn.PublicConn);
            if (ds != null)
